...
public class SmokeFluidSim : MonoBehaviour
{
	...
	void Start() {
		...
		m_velocityField = new RenderTexture(m_width, m_height, 0, RenderTextureFormat.ARGBFloat);
		m_velocityField.enableRandomWrite = true;
		m_velocityField.dimension = UnityEngine.Rendering.TextureDimension.Tex3D;
		m_velocityField.volumeDepth = m_depth;
	...
	void ComputeVelocityField() {
		m_computeVelocityField.SetVector("_Size", m_size);
		...
		m_computeVelocityField.Dispatch(0, (int)m_size.x / NUM_THREADS, (int)m_size.y / NUM_THREADS, (int)m_size.z / NUM_THREADS);
	}
